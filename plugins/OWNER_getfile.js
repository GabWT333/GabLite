...
// Funzione per parsare multiple richieste di plugin
function parseMultiplePlugins(text) {
  // Rimuove eventuali opzioni globali alla fine
  const cleanText = text.replace(/\s+(script|file)\s*$/i, '');
  
  // Divide il testo usando '&' come separatore
  ...
  for (const part of parts) {
    if (part.length > 0) {
      ...
      plugins.push({
        name: args[0],
        option: args[1] || null // Non impostare default qui
      });
    }
  }
  
  return plugins.slice(0, 3); // Massimo 3 plugin
}

// Funzione per determinare l'opzione globale dal comando
...
Esempi multipli (solo per plugin):
  ...
  // Controlla se è una richiesta multipla (contiene '&')
  if (isPlugin && text.includes('&')) {
    ...
    // Se c'è un'opzione globale, la applica a tutti i plugin che non hanno già un'opzione
    if (globalOption) {
    ...
    // Controlla se tutti i plugin hanno un'opzione specificata
    const hasAllOptions = plugins.every(p => p.option);
    
    if (!hasAllOptions) {
      // Se non hanno specificato opzioni per tutti, mostra i pulsanti
      if (global.getMultiFileData) {
        delete global.getMultiFileData;
      }
      
      // Crea una stringa base dei plugin per i pulsanti
      ...
      return await conn.sendButton(m.chat, info, '333 Multi Plugin Manager', null, buttons, m)
    }
  ...
  if (args.length < 2) {
    ...
    return await conn.sendButton(m.chat, info, '333 File Manager', null, buttons, m)
  }
